# Concepts

Shared language for MindFree. Read these before diving into architecture ADRs or entity file maps.

| Doc | Purpose |
| --- | ------- |
| [glossary.md](./glossary.md) | Product terms — note kinds, flags, read models |
| [terminology.md](./terminology.md) | Codebase terms — FSD layers, `tanstack/`, etc. _(Phase 5)_ |

**Related:** [entities/note/docs/domain-model.md](../../entities/note/docs/domain-model.md) expands note concepts with lifecycle rules.
